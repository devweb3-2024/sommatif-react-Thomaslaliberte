...
const GrilleMot: React.FC<GrilleChatsProps> = ({}) => {
  ...
  function clique(caseChoisie: number) {
    // ER: En React, on ne doit pas modifier les variables directement, ça doit passer par un état. (useState)
    if (choix == '') {
  ...
  return (
    <>
      ...
      <Grid
        container
        spacing={1}
        sx={{ marginTop: 2, width: 900, justifyContent: 'center' }}
      >
        {chats &&
          tableau.map((tableau) => {
            return (
              <Grid>
                ...
                >
                  <Button
                    variant="contained"
                    onClick={() => clique(tableau)}
                    sx={{ height: 150, width: 150 }}
                    disabled={partieTermine}
                  >
                    <img
                      src={partieTermine ? chats[tableau] : imageDefaut}
                      alt=""
                      height={150}
                      width={150}
                    />
